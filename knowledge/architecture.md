...
4. **Static File Exclusions**: Networks parsed natively from the `--static` file block via `std.Io.net.IpAddress` are inserted into the tree as `HOLE` components immediately prior to optimization, forcefully carving them out of the CSV data.
5. **Bottom-Up Optimization**: 
...
6. **Dumping**: A pre-order traversal traverses the optimized tree, emitting perfectly disjoint non-overlapping CIDR blocks.
